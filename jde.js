                            ...
                            AwsDB( qString, req, res, function(result) {
                                if( result.rowsAffected == 0){
									...
									if (CustName != "" && CustName != null) {
										...
										qString = "Select * from jde WHERE REPLACE(REPLACE(REPLACE(CustName, '.',''), '/', ''), '\,', '') LIKE '" + CustName + "%'";
										AwsDB( qString, req, res, function(result1) {
											if( result1.rowsAffected == 0){
												qString = "Select * from jde WHERE REPLACE(REPLACE(REPLACE(CustName, '.',''), '/', ''), '\,', '')  LIKE '" + CustName.substr(0, (CustName.length)/2) + "%'";
													AwsDB( qString, req, res, function(result1) {
														if( result1.rowsAffected == 0){
															speechText = "No records found.";
														}
                        ...
                        if(qString != ""){
                            AwsDB( qString, req, res, function(result) {
                                if( result.rowsAffected == 0){
									...
									if (CustName != "" && CustName != null) {
										console.log("CustName : " + CustName.length);
										qString = "Select * from jde WHERE CustName  LIKE '" + CustName + "%'";
										AwsDB( qString, req, res, function(result1) {
											if( result1.rowsAffected == 0){
